...
### 5.15 免 root 抓包(VPNService)

| 项目 | 说明 |
|---|---|
| 引擎 | 系统 VPN 通道(`VpnService`)接管全部流量,进程内解析 IP 包 —— **无需 root** |
| 协议 | IPv4/IPv6 → TCP / UDP / ICMP;分协议包数与字节统计 |
| 流表 | TCP/UDP 五元组连接流(客户端/服务端、收发字节、包数、SYN/FIN/RST 状态),45s 空闲过期 |
| DNS | 客户端→53 查询域名解码(支持压缩指针的 label 解析) |
...
### 5.16 局域网 Web 控制台(内置 HTTP 服务器)

| 项目 | 说明 |
|---|---|
| 传输 | 极简 HTTP 服务器(ServerSocket,零依赖),端口 8080,仅限局域网 |
| 仪表盘 | `/` 深色主题控制台:报告列表(质量徽章)、实时抓包统计、探测能力 |
| API | `/api/reports`、`/api/report/{id}`、`/api/capture`、`/api/capabilities`(JSON) |
| 下载 | `/download/report/{id}`、`/download/samples/{id}/…`(CSV)、`/download/pcap` |
| 远程扫描 | `POST /api/scan`(ids + duration)→ 前台服务执行测量会话,报告入历史 |
| 提示 | ⚠️ 仅限可信局域网,不要暴露到公网 |

### 5.17 实时监测

| 项目 | 说明 |
|---|---|
| 示波器 | 实时波形(FASTEST 采样):加速度/陀螺/磁力/光照/温度/气压/声压级;800 点环形缓冲,3 通道(x/y/z) |
| 频谱瀑布 | AudioRecord 4096 点 FFT → 64 个对数幅度 bin(0-8kHz),40 行瀑布图 |
| 告警 | 阈值(噪音 dB(A)/温度 °C/光照 lx)每 120ms 检查 → 高优先级通知(同一指标 5 秒内最多一次),可配置可开关 |

### 5.18 传感器标定向导

| 步骤 | 流程 | 输出 |
|---|---|---|
| 1. 磁力计 | 手持画 8 字覆盖所有姿态(约 20 秒) | 硬铁偏移 x/y/z(µT)、幅值范围 |
