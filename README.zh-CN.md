...
- **测量目录(61 项)**: 每项探测都是正式的 `ProbeSpec` 条目,定义被测量、SI 单位、标称采样率、量程 —— 覆盖运动学、环境物理量、磁场、生物信号、声学、定位与卫星、无线电、电气、系统资源、设备信息、上下文事件
...
>
> **合规风险标注项**:以下探测项采集的数据在部分司法辖区受监管或属个人数据,应用内与报告中均以 ⚠️ 标注:WiFi 扫描/RTT/Direct/Aware、蓝牙扫描/经典发现/已配对列表、蜂窝与小区标识、位置与 GNSS(含原始观测量)、麦克风声压级、NFC、心率、设备序列号。详见 [docs/PROBES.zh-CN.md](docs/PROBES.zh-CN.md) §7.5。
